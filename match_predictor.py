...
def predict_draft(knn_model, ally_draft, enemy_draft, team_id, unique_team_ids):
    """
    Predict win probability or class for a given draft + team_id.

    :param knn_model: trained KNeighborsClassifier
    :param ally_draft: list of ally champion_ids (1-indexed)
    :param enemy_draft: list of enemy champion_ids (1-indexed)
    :param team_id: integer team ID
    :param unique_team_ids: list of all unique team_ids (from training data)
    ...
    # Team one-hot
    team_one_hot = np.zeros((1, len(unique_team_ids)), dtype=np.int32)
    team_idx = unique_team_ids.index(team_id)
    team_one_hot[0, team_idx] = 1

    # Combine draft + team
    x_input = np.hstack([draft_vec, team_one_hot])
...
def main():
    ...
    # Get unique team_ids (needed for one-hot at prediction time)
    predict_df = pd.read_csv("data/train_data.csv")
    unique_team_ids = sorted(predict_df["team_id"].unique())
    ...
    ally_draft = ["Rumble", "Sejuani", "Yone", "Varus", "Karma"]  # champion names
    enemy_draft = ["Ambessa", "Wukong", "Azir", "Corki", "Bard"]
    team_id = 1
